...
const firebaseConfig = {
  apiKey: "AIzaSyCgYaNTikl0FxhX2xDQl_oVaX7fi8Iko58",
  authDomain: "ecom-tursm.firebaseapp.com",
  projectId: "ecom-tursm",
  storageBucket: "ecom-tursm.appspot.com",
  messagingSenderId: "266525926521",
  appId: "1:266525926521:web:7ac32de7d12501711c60a8"
};
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// generate image url Genera una URL del upload a AWS de las imagenes y las devuelve
...
app.post('/signup', (req, res) => {
    ...
    // form validations
    if(name.length < 3){
        res.json({ 'alert' : 'El nombre debe tener mas de tres letras'});
    } else if(!email.length){
        res.json({ 'alert' : 'Entre su email'});
    } else if(password.length < 8){
        res.json({ 'alert' : 'Password debe tener 8 letras'});
    } else if(!Number(number) || number.length < 10){
        res.json({ 'alert' : 'Numero no valido'});
    } else if(!tac){
        res.json({ 'alert' : 'Debe aceptar los terminos y condiciones'});
    } else{
        ...
        getDoc(doc(users, email)).then(user => {
            if(user.exists()){
                return res.json({ 'alert' : 'email ya existe' })
            } else{
...
app.post('/seller', (req, res) => {
    ...
    if(!name.length || !address.length || !about.length || number.length < 10 || !Number(number)){
        return res.json({ 'alert' : 'Informacion incorrecta' });
    } else{
        // update seller
...
app.post('/add-product', (req, res) => {
    ...
    if(!draft){
        if(!name.length){
            res.json({'alert' : 'Ingrese el nombre del producto'});
        } else if(!shortDes.length){
            res.json({'alert' : 'Titulo debe tener menos de 80 letras'});
        } else if(!price.length || !Number(price)){
            res.json({'alert' : 'Entre un valor valido para el precio'});
        } else if(!detail.length){
            res.json({'alert' : 'Faltan describir en deta;;es'});
        } else if(!tags.length){
    ...
    .catch(err => {
        res.json({'alert': 'Ocurrio un error'})
    })
})
...
app.post('/add-review', (req, res) => {
    ...
    if(!headline.length || !review.length || rate == 0 || email == null || !product){
        return res.json({'alert':'LLene todos los campos'});
    }

    // Send/save to Firestore
